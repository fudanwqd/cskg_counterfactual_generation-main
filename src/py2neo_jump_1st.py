...
relations_with_sentece_matcher = RelationshipMatcher(graph)
# relations_with_sentece = (relations_with_sentece_matcher.match(None, None).limit(5)).where(sentence='NoneSentence')
# "_.sentence =~ 'NoneSentence'"
relations_with_sentece = relations_with_sentece_matcher.match(None, None).where("_.sentence <> 'NoneSentence'").limit(100)
...
for relation_with_sentece in list(relations_with_sentece):
    # print(relation_with_sentece.start_node)
    # print(relation_with_sentece.end_node)
    # print(relation_with_sentece.nodes)
    # print(relation_with_sentece.relationships)  # (related to(Node('Entity', content='0', entityID='/c/en/0'), Node('Entity', content='2', entityID='/c/en/2'), sentence='NoneSentence'),)
    # print(relation_with_sentece.identity)
...
def get_random_relation(relation_dict):
    ...
    # (_0)-[:is a {sentence: '[[{0}]] is a type of [[set containing one element]].'}]->(_6)
    random_relation = relations_with_sentece_matcher.match(None, r_type=relation_name_key).first()
    # [[{0}]] is a type of [[set containing one element]].
...
for i in range(5):
    random_subject_set, random_object_set, random_sentence = get_random_relation(relation_dict)

    random_replaced_subject_content = get_random_subject_content(random_subject_set)
    random_replaced_object_content = get_random_object_content(random_object_set)
